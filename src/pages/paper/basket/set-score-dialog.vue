  ...
  <el-dialog title="编辑题号分数" :visible.sync="visible" width="500px" :before-close="handleClose" custom-class="m-dialog" class="set-score-dialog" center>
    ...
    <div class="set-score-box">
      <div class="set-score-item">
        <div>题号
          <el-input size="mini" class="ipt-idx" v-model="parentScore.showNo"></el-input>
        </div>
        <div>分数
          <el-input size="mini" class="ipt-score" @change="parentScoreChange(parentScore)" v-model.number="parentScore.itemScore" :disabled="childsScore.length > 0" type="number" step="0.1"></el-input>
        </div>
      </div>
    </div>
    <template v-if="childsScore.length > 0">
      ...
      <div class="set-score-box">
        <div class="set-score-item" v-for="item in childsScore" :key="item.itemId">
          <div>题号
            <el-input size="mini" class="ipt-idx" v-model="item.showNo"></el-input>
          </div>
          <div>分数
            <el-input size="mini" class="ipt-score" v-model.number="item.itemScore" @change="childScoreChange(item)" type="number" step="0.1"></el-input>
          </div>
        </div>
      </div>
    </template>
